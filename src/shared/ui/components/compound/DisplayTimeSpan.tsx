...
export function DisplayTimeSpan({ startValue, stopValue }: Props) {
  ...
  return (
    <div className="flex gap-2 items-center">
      {(difference.hours) ?
        <div>{difference.hours} <span className="text-fg-muted">{hourLabel}</span></div> : null
      }
      {(difference.minutes) ?
        <div>{difference.minutes} <span className="text-fg-muted">{minuteLabel}</span></div> : null
      }
      {(difference.seconds) ?
        <div>{difference.seconds} <span className="text-fg-muted">{secondLabel}</span></div> : null
      }            
      {(difference.milliseconds) ?
        <div>{difference.milliseconds} <span className="text-fg-muted">{millisecondLabel}</span></div> : null
      }
    </div>
  )
}