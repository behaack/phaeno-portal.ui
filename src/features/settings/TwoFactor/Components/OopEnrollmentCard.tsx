  ...
  const step1 = (
    <div>
      ...
      <div className="text-right mt-5">
        <PButton onClick={sendCode} disabled={!password.length} color="black">
          Send Code
        </PButton>
      </div>
    </div>
  ...
  const step2 = (
    <div>
      <p className="text-sm mb-3 text-gray-600">
        <b>Step 2:</b> Check you email for 6-digit code, then confirm it here.
      </p>
      <image />
      <PTextInput
        value={code}
      ...
      />
      <div className="text-right mt-5">
        <PButton px={30} onClick={confirmCode} disabled={code.length < 6}>
          Finish
        </PButton>
      </div>
    </div>
