...
import { NaturalLangRequest, type AiAssistResponse } from '../types/ai-assistant'
...
type PageParam = { mode: 'cursor'; cursor: string | null } | { mode: 'page'; pageNo: number }
...
export function useAiAssistInfinitePages(opts: {
  ...
  return useInfiniteQuery<AiAssistResponse, Error, { pages: AiAssistResponse[] }, any, PageParam>({
    ...
    getNextPageParam: (lastPage, allPages) => {
      ...
      return lastPage.nextCursor ? { mode: 'cursor', cursor: lastPage.nextCursor } : undefined
    },
  })
}
